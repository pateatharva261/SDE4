  ...
  parts: [
    {
      id: "part-01",
      num: 1,
      title: "The Mindset of System Design",
      status: "done",
      modules: [
        {
    ...
    },
    {
      id: "part-19",
      num: 19,
      title: "Interview System Designs",
      status: "done",
      modules: [
        {
...
// contentlayer.config.ts
function getDocMetadata(sourceFilePath, rawContent) {
  ...
  for (const part of COURSE.parts) {
    for (const mod of part.modules) {
      ...
      if (lessonItem) {
        const desc = new RegExp(`\\*\\*Description:\\*\\*\\s*(.+)`, "i").exec(rawContent.slice(0, 2e3))?.[1]?.trim() || "";
...
var preMermaidPlugin = () => (tree) => {
  const visit = (node) => {
    if (node.tagName === "pre" && node.children?.[0]?.tagName === "code" && node.children[0].properties?.className?.includes("language-mermaid")) {
...
//# sourceMappingURL=compiled-contentlayer-config-3VF6SHKN.mjs.map
